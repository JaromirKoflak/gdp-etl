...
GDP deflators are used for estimating the **nominal** GDP for the year `r last_year`. This report compares the GDP deflators in the years preceding `r last_year`. 

- `Deflator_DSIB` shows GDP deflators sourced from UNCTAD DSIB. Deflators of the year `r last_year` are used for the estimation.
- `Deflator_UNSD` is computed separately using data from this ETL script.

The following table shows the discrepancies in the last non-estimated year `r last_year-1`.
...
The following table highlights the largest discrepancies found for each economy before `r last_year`.
